export enum DiplomaType {
  SECONDARY_SCHOOL_CERTIFICATE = 'Атестат про повну загальну середню освіту',
  JUNIOR_BACHELOR_DIPLOMA = 'Диплом молодшого спеціаліста',
  BACHELOR_DIPLOMA = 'Диплом бакалавра',
  MASTER_DIPLOMA = 'Диплом магістра',
  PHD_DIPLOMA = 'Диплом доктора філософії',
  DOCTOR_DIPLOMA = 'Диплом доктора наук',
  OTHER_FOREIGN = 'Іноземний документ',
  QUALIFIED_WORKER_DIPLOMA = 'Диплом кваліфікованого робітника',
  WORKER_QUALIFICATION_DIPLOMA = 'Свідоцтво про присвоєння(підвищення) робітничої кваліфікації',
  SPECIALIST_DIPLOMA = 'Диплом спеціаліста'
}
